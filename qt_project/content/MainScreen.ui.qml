...
Pane {
    ...
    Connections {
        target: backend
        //processOutput: voiceCommand.text = output
    }
    ...
    ColumnLayout {
        ...
        TextInput {
            id: ipAddress
            text: qsTr("192.168.1.11")
            Layout.alignment: Qt.AlignHCenter | Qt.AlignVCenter
        }

        Button {
            id: connectButton
            text: qsTr("Connect to Robot")
            Layout.alignment: Qt.AlignHCenter | Qt.AlignVCenter

            Connections {
                target: connectButton
                onClicked: backend.connectToRobot(ipAddress.text)
            }
        }
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Button {
                id: startProcess
                text: qsTr("Start Voice Control")

                Connections {
                    target:startProcess
                    onClicked: backend.runProcess()
                }
            }
        }
    ...
    FileDialog {
        ...
        Connections {
            target: fileDialog
            onAccepted: backend.scriptPath = fileDialog.selectedFile.toString().replace("file://", "")
        }
    }
...
/*##^##
Designer {
    D{i:0}D{i:33;cameraSpeed3d:25;cameraSpeed3dMultiplier:1}
}
##^##*/
